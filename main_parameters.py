...
############################
SEC_USER_AGENT = (
    "Fovi LLC jim@fovi.com"  # <-- Your info here. Required by SEC's EDGAR system
)
SEC_HTTP_HEADERS = {
...
sec_master_url_template = (
    """edgar/full-index/{year}/QTR{quarter}/master.idx"""  # dl_idx.py
)
...
FILINGS_DIRECTORIES = [
    DATA_RAW_FOLDER / f"{x}_filings" for x in FILING_TYPES
]  # e.g. ['13D_filings', '13G_filings', ...]
...
FILTERED_INDEX_FILE = (
    DATA_PROC_FOLDER / "master_index_filtered.csv"
)  # historical archive, filtered for chosen filing types (write: dl_idx.py, read: dl2.py)
...
):  # <-- test period. Widest should be (1994, current_year+1)
    start_qrt = START_QUARTER if year == START_YEAR else 1
    for quarter in range(start_qrt, 4 + 1):  # <-- test period. Otherwise range(1, 4+1)
        if (year == current_year and quarter > current_quarter) or (
            year > current_year
        ):
            break
        SEC_MASTER_URLS.append(
            sec_master_url_template.format(year=year, quarter=quarter)
        )
