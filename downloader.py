...
from main_parameters import SEC_HTTP_HEADERS, HTTP_TIMEOUT, SEC_RATE_LIMIT, MAX_CHUNKSIZE
...
def download_file(row):
    ...
    try:
        ...
        while "Request Rate Threshold Exceeded" in response.text:
            logger.warning("Rate limit exceeded. Waiting for 615 seconds.")
            time.sleep(615)
            response = requests.get(
                url=url, headers=SEC_HTTP_HEADERS, timeout=HTTP_TIMEOUT
            )
...
def download_files(rows: Sequence, overwrite: bool = False) -> None:
    ...
    with ProcessPoolExecutor(max_workers=SEC_RATE_LIMIT, initializer=init_worker, initargs=(log_queue, log_level)) as executor:
        executor.map(
            download_file,
            rows,
